...
# The six published proof books reserve $1.2k in aggregate. Keep the staging
# fixture budget equal to that declared workload so the Phase 7 legs are
# reached; production budget enforcement remains covered by its own tests.
export DAILY_SEED_BUDGET_MICRO=1200000000
...
# The server sees this harness through trusted loopback, so the pinned
# TEST-NET-3 hop is the client IP; without it D33 correctly refuses before any
# warmth, limit, AML or self-exclusion assertion can run.
withdraw_request() {
...
# stub), so this economically complete fixture seeds its balanced credit
# issuance and cash reserve backing as well as the lot. Everything asserted
# below is production behaviour: fee allocation, the pre-Paid no-convert rule,
# and the post-Paid lazy convert.
sql "begin;
  insert into ledger_accounts(owner_type,owner_id,currency)
  values ('external',null,'usdc'), ('external',null,'usdc_credit')
  on conflict (currency) where owner_type='external' do nothing;

  insert into ledger_accounts(owner_type,owner_id,currency)
  values ('house',null,'usdc_credit'), ('bonus_reserve',null,'usdc')
  on conflict (owner_type,currency)
    where owner_type in ('fees','house','withheld','deposit_suspense','bonus_reserve')
    do nothing;

  insert into ledger_accounts(owner_type,owner_id,currency)
  values ('user','$GRANT_USER','usdc_credit')
  on conflict (owner_type,owner_id,currency)
    where owner_type in ('user','pool','escrow') do nothing;

  with fixture_seed as (
    insert into ledger_transactions(kind,idempotency_key)
    values ('seed','$RUN_TAG-bonus-fixture-seed') returning id
  )
  insert into ledger_entries(txn_id,account_id,amount_micro)
  select id,(select id from ledger_accounts where owner_type='external' and currency='usdc'),-$BONUS_LOT
    from fixture_seed
  union all
  select id,(select id from ledger_accounts where owner_type='bonus_reserve' and currency='usdc'),$BONUS_LOT
    from fixture_seed
  union all
  select id,(select id from ledger_accounts where owner_type='external' and currency='usdc_credit'),-$BONUS_LOT
    from fixture_seed
  union all
  select id,(select id from ledger_accounts where owner_type='house' and currency='usdc_credit'),$BONUS_LOT
    from fixture_seed;

  with fixture_grant as (
    insert into ledger_transactions(kind,idempotency_key)
    values ('credit_grant','$RUN_TAG-bonus-fixture-grant') returning id
  )
  insert into ledger_entries(txn_id,account_id,amount_micro)
  select id,(select id from ledger_accounts where owner_type='house' and currency='usdc_credit'),-$BONUS_LOT
    from fixture_grant
  union all
  select id,(select id from ledger_accounts where owner_type='user' and owner_id='$GRANT_USER' and currency='usdc_credit'),$BONUS_LOT
    from fixture_grant;

  insert into credit_grant_lots(
    user_id,source,amount_micro,grant_class,policy_version,idempotency_key
  ) values (
    '$GRANT_USER','signup',$BONUS_LOT,'real_money','e2e','$RUN_TAG-bonus-fixture-lot'
  );
  commit;" >/dev/null
prereq_missing "credit grant issuance: /admin/credits/grant/{propose,confirm} are phase7_admin_stubs 503s, so production reserve-at-grant encumbrance cannot be exercised and the balanced grant/reserve state must be seeded"
