...
test("mobile vote + crowd guess -> trade -> payout -> share-card img", async ({
  ...
  // A reload discards the in-memory vote receipt. The server-scoped vote read
  // must still restore the gate without revealing the live vote's side.
  await page.reload();
  await expect(page.getByText("Vote recorded.", { exact: true })).toBeVisible();
  await expect(page.getByText("Unlock trading by voting.")).toHaveCount(0);
  await expect(page.getByRole("button", { name: "Preview ticket" })).toBeEnabled();
