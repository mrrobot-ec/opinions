...
export default function MarketDetailPage() {
  ...
  async function onConfirmTrade() {
    ...
    try {
      ...
      setTradeOk(
        `Filled ${formatShares(receipt.shares_micro)} shares @ ${formatCentsPrice(receipt.avg_price_micro)}`,
      );
      setPreview(null);
      idemRef.current = newIdempotencyKey();
    } catch (e) {
  ...
  const flashCls =
    priceFlash === "up"
      ...
      : priceFlash === "down"
        ...
        : "";
  const myPnl = myPositions.reduce((s, p) => s + p.realized_pnl_micro, 0);
            ...
            <div className="panel terminal-screen">
              ...
              {userId && (
                <div style={{ marginTop: "1.25rem" }}>
                  ...
                  <div
                    className={`big num ${myPnl >= 0 ? "yes-tone" : "no-tone"}`}
