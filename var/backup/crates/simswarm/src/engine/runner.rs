...
pub struct Runner {
    ...
    /// Optional HTTP-send timestamps consumed by a WS collector as
    /// send → frame delivery samples.
    pub http_send_marks: Arc<std::sync::Mutex<Vec<Instant>>>,
}
...
impl Runner {
    ...
    pub async fn run_planned(
        ...
        for opportunities in by_actor.into_values() {
            let worker = ActorWorker {
                ...
                http_send_marks: Arc::clone(&self.http_send_marks),
            };
    ...
    /// Consume one HTTP-send mark and record a WS delivery sample.
    pub fn record_ws_delivery(&self, latency: &mut LatencyLog, delivered_at: Instant) {
        let sent = self
            .http_send_marks
            .lock()
        ...
        if let Some(sent) = sent {
            let ms = u64::try_from(delivered_at.duration_since(sent).as_millis()).unwrap_or(0);
            latency.record("ws-delivery", 200, ms);
        }
    }
}
...
struct ActorWorker {
    ...
    max_retries: u8,
    http_send_marks: Arc<std::sync::Mutex<Vec<Instant>>>,
}
    ...
    async fn run(self, opportunities: Vec<PlannedOpportunity>) -> Result<ActorReport, SwarmError> {
        ...
        while let Some((opportunity, attempt)) = queue.pop_front() {
            ...
            for (index, agent_action) in decision.actions.iter().enumerate() {
                ...
                // Only book-moving actions fan out a public WS frame; a
                // withdraw request does not.
                if matches!(operation, "trade-confirm" | "vote") && (200..300).contains(&status) {
                    latency.record("ws-delivery", 200, elapsed);
                }
                outcomes.extend(one);
            }
...
mod tests {
    ...
    async fn one_actor_task_sleeps_in_sim_time_and_records_separate_latency() {
        ...
        let runner = Runner {
            ...
            risk_hold_ms: 0,
            lifecycle_polls: 0,
            http_send_marks: Arc::new(Mutex::new(Vec::new())),
        };
        let report = runner
            .run_planned(
                &manifest,
    ...
    async fn live_close_to_paid_subtracts_the_named_risk_hold() {
        ...
        runner.record_ws_delivery(&mut latency, Instant::now());
        assert_eq!(latency.successful("ws-delivery").len(), 1);
        runner.record_ws_delivery(&mut latency, Instant::now());
        assert_eq!(latency.successful("ws-delivery").len(), 1);
    }
    ...
    #[tokio::test]
    async fn paid_without_prior_closed_still_records_the_gated_timer() {
