...
use crate::money::{
    allows_progress, AllocationFact, AmlDirection, AmlFlag, AmlLeg, AmlPolicy,
    ComplianceDecision, ConvertCollectReceipt, CreditConversionTxn, CreditIo, CreditLotRow,
    DepositMachineRow, DepositRefundPayment, ObservedDeposit, ReferralPaidMarket,
};
...
impl CreditIo for InMemTx {
    ...
    async fn lot_by_idempotency(&mut self, key: &str) -> Result<Option<CreditLotRow>, StoreError> {
        ...
        Ok(st.phase7.lot_keys.get(key).and_then(|id| {
            st.phase7.lots.iter().find(|lot| lot.id == *id).cloned()
        }))
    }
    ...
    async fn mark_lot_converted(&mut self, lot: Uuid, at: OffsetDateTime) -> Result<(), StoreError> {
    ...
    async fn insert_allocation(&mut self, fact: &AllocationFact) -> Result<(), StoreError> {
        ...
        let valid = match fact.kind {
            ...
            crate::money::AllocationKind::Finalized
            | crate::money::AllocationKind::Reversed => {
    ...
    async fn insert_refund_payment(
        ...
        let id = Uuid::new_v4();
        self.pending.phase7.outbound_payments.push(crate::ports::OutboundPaymentRow {
            id,
    ...
    async fn referral_code_for_user(
        &mut self,
        user: UserId,
...
impl DepositAmlIo for InMemTx {
    async fn evaluate_deposit_aml_candidate(
        ...
        let (committed_legs, committed_flags) = {
            let state = self.shared.state.lock();
            (state.phase7.aml_legs.clone(), state.phase7.aml_flags.clone())
        };
        let recorded = self
            .pending
            .phase7
...
mod credit_fake_semantics_tests {
    ...
    async fn pending_credit_views_filter_users_and_terminal_allocations() {
        ...
        assert_eq!(tx.remember_lot_idempotency("own-lot", own_lot.id).await, Ok(()));
        assert_eq!(tx.remember_lot_idempotency("orphan-lot", id(999)).await, Ok(()));
        assert_eq!(tx.lots_for_user(user).await, Ok(vec![own_lot.clone()]));
        assert_eq!(tx.lot_by_idempotency("own-lot").await, Ok(Some(own_lot.clone())));
    ...
    async fn pending_deposit_refund_and_referral_reads_are_transaction_local() {
        ...
        assert_eq!(tx.referral_code_for_user(user).await, Ok(Some("REFCODE".into())));
        ...
        assert_eq!(tx.referral_bind_by_key("pending-bind").await, Ok(Some(bind)));
        assert_eq!(
    ...
    async fn outbound_lineage_reads_pending_updates_and_rejects_duplicate_intents() {
        ...
        assert_eq!(
            tx.outbound_by_subject(payment.subject, payment.subject_id).await,
        ...
        assert_eq!(tx.attempts_for(payment.id).await, Ok(vec![broadcast.clone()]));
        assert_eq!(tx.commit().await, Ok(()));
        ...
        assert_eq!(recovery.live_attempt(payment.id).await, Ok(Some(replacement.clone())));
        assert_eq!(
