...
use crate::model::{
    ...
    RealizationFact, RealizationSource, RepConfig, ReputationRow, ResolveConfig, VoteScoreUpdate,
};
...
impl<S: Store, C: Clock> ResolveMarket<'_, S, C> {
    ...
    pub async fn execute(&self, cmd: ResolveMarketCmd) -> Result<ResolveReceipt, AppError> {
        ...
        let mut voter_ids = tx.voter_ids(cmd.market).await?;
        voter_ids.sort_unstable_by_key(|user| user.0);
        voter_ids.dedup();
        // D32 referral-on-Paid: referral-relevant users (referrer + referee of
        // potentially-qualifying binds) take their advisory user locks BEFORE
        // the market lock. Include unbound qualifying referees so a concurrent
        // bind cannot appear after enumeration without sharing their lock.
        // Voters and referral users form ONE globally sorted lock run; two
        // independently sorted runs can invert the class-2 order and deadlock.
        let mut referral_users = tx.referral_relevant_users(cmd.market).await?;
        referral_users.sort_unstable_by_key(|user| user.0);
        referral_users.dedup();
        let mut lock_users = voter_ids.clone();
        lock_users.extend(referral_users);
        lock_users.sort_unstable_by_key(|user| user.0);
        lock_users.dedup();
        let locked_reps: Vec<_> = tx
            .reps_for_update(&lock_users)
            .await?
            .into_iter()
            .filter(|row| {
                voter_ids
        ...
        let outcome = if voided {
            ...
            let paid =
                transition(resolved, MarketEvent::Pay).map_err(|_| AppError::IllegalTransition)?;
