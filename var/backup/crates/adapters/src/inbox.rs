...
use application::money::admin::{
    accept_inbox, hex_encode, payload_hash, ComplianceAdminStore, InboxOutcome, InboxRecord,
};
...
/// Ingest one webhook: verify, resolve user, persist algebra.
///
/// # Errors
...
pub async fn ingest(
    ...
    let outcome = accept_inbox(store, record).await;
...
mod tests {
    ...
    async fn ingest_accepts_replay_and_pages_on_conflict() {
        ...
        let first = ingest(&store, &UnavailableMoney, secret, evt1, user, now)
            .await
            .unwrap();
        assert_eq!(first, InboxOutcome::Accepted);
        let replay = ingest(&store, &UnavailableMoney, secret, evt1, user, now)
            .await
            .unwrap();
        ...
        let conflict = ingest(&store, &alerter, secret, clashing, user, now).await;
        assert!(matches!(
            conflict,
        ...
        assert!(matches!(
            ingest(&store, &UnavailableMoney, secret, clashing, user, now).await,
