...
impl InvariantReadTx for PgInvariantTx {
    ...
    async fn unbalanced_txns(&mut self) -> Result<Vec<TxnSumRow>, StoreError> {
        let rows = sqlx::query(
            "select txn_id, sum(amount_micro)::bigint as sum_micro
               from ledger_entries group by txn_id having sum(amount_micro) <> 0",
        )
    ...
    async fn account_balances(&mut self) -> Result<Vec<AccountBalanceRow>, StoreError> {
        let rows = sqlx::query(
            "select a.owner_type, coalesce(sum(e.amount_micro), 0)::bigint as balance_micro
               from ledger_accounts a
               left join ledger_entries e on e.account_id = a.id
              group by a.id, a.owner_type",
        )
        .fetch_all(&mut *self.tx)
        .await
        .map_err(db_error)?;
