...
async fn main() -> anyhow::Result<()> {
    ...
    // SharedAlerter + NoopTelemetry always compose; the pinned chain-balance
    // read exists only under a validated rail identity (staging pin via env);
    // the at-least-once incident delivery pump runs every minute over the
    // alert store (MemoryAlertStore until W4's Pg AlertStore lands on the
    // 0011 delivery/ack/resolve columns).
    // Phase 7 money observability (W4 finalized ask, coordinator-applied):
    // the durable Pg alert store carries D35's at-least-once delivery
    // bookkeeping and incident lifecycle over the 0011 alert_outbox columns.
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(error) = manager.deliver_pending().await {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match application::integrity::invariant_sweep::run(&sweep_store).await {
                    ...
                    Ok(report) => eprintln!("invariant sweep: VIOLATION {report:?}"),
    ...
    let region_allowset_version: i64 = env::var("REGION_ALLOWSET_VERSION")
        .ok()
