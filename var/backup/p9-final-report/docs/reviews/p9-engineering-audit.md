...
currency accounting, deposit AML enforcement, browser withdrawal replay,
production alert wiring, WebSocket delivery latency, and several tests that
could report success without exercising their stated contract. Each production
repair below has a retained discriminating regression; several concurrency and
database findings were also independently refuted or confirmed before repair.
...
recorded 496 real delivery samples at p50/p95/p99 33/57/78 ms and passed the
...
- the Phase-7 swarm smoke accepted 404/503, discarded HTTP statuses, omitted
  ...
  effects. Its 40 integrity tests execute status helpers, require positive
...
The final live runs caught five additional verification defects rather than
...
exercised. The retained 40th smoke-integrity test failed first, then the helper
...
4 live-loop, 1 Pg-suite, 5 shell, 40 smoke and 3 static-quality regressions
