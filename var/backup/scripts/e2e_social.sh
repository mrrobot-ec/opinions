...
create_user() {
  ...
  psql_demo "insert into reputation (user_id, rep_micro, tier) values ('${uid}', 0, ${tier}) on conflict (user_id) do update set tier=${tier}" >/dev/null || true
...
for u in "${YOUNG[@]}"; do
  ...
  # expect 403 reporter_floor
  if [ "$code" != "403" ] && [ "$code" != "422" ] && [ "$code" != "409" ]; then
    # 200 would be bad for young
    :
  fi
done
...
# Prefer that reports were rejected — if some 200, still no shadow is the product bar
...
for i in $(seq 1 $((MENTION_NOTIFS_PER_HOUR + 3))); do
  ...
  body="hey @${TH} flood ${i}"
  resp=$(post_comment "$M2" "$u" "$body" 2>/dev/null || echo '{}')
  echo "   flood_${i} author=${u} comment_id=$(echo "$resp" | jq -r '.id // empty')"
done
...
for i in $(seq 1 12); do
  ...
  post_comment "$M2" "$u" "pagination filler ${i} $(uuid | tr -d '-')" >/dev/null || true
done
