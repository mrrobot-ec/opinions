...
struct ReferralGrantPolicy {
    minimum_notional_micro: i64,
    referrer_amount_micro: i64,
    referee_amount_micro: i64,
    grant_class: GrantClass,
    mint_cap_micro: i64,
}

async fn required_referral_policy<T: CreditIo + ?Sized>(
    tx: &mut T,
) -> Result<ReferralGrantPolicy, StoreError> {
    let required = || StoreError::Invariant("required money config missing or malformed");
    let minimum_notional_micro = tx
        .config_i64("referral_min_notional_micro")
        .await?
        .ok_or_else(required)?;
    let referrer_amount_micro = tx
        .config_i64("credit_referral_referrer_micro")
        .await?
        .ok_or_else(required)?;
    let referee_amount_micro = tx
        .config_i64("credit_referral_referee_micro")
        .await?
        .ok_or_else(required)?;
    let grant_class = match tx.config_text("bonus_structure").await?.as_deref() {
        Some("real_money") => GrantClass::RealMoney,
        Some("sweeps") => GrantClass::Sweeps,
        Some(_) => return Err(StoreError::Invariant("invalid persisted bonus structure")),
        None => return Err(required()),
    };
    let mint_cap_micro = tx
        .config_i64("bonus_mint_daily_cap_micro")
        .await?
        .ok_or_else(required)?;
    Ok(ReferralGrantPolicy {
        minimum_notional_micro,
        referrer_amount_micro,
        referee_amount_micro,
        grant_class,
        mint_cap_micro,
    })
}
...
impl<S: Store> GrantReferral<'_, S> {
    ...
    pub async fn execute(&self, cmd: GrantReferralCmd) -> Result<GrantReferralReceipt, AppError> {
        ...
        let policy = required_referral_policy(tx.as_mut()).await?;
        for user in users {
        ...
        let paid = tx
            ...
            .await?
            .ok_or(AppError::ReferralIneligible)?;
        if paid.market != cmd.paid_market || paid.notional_micro < policy.minimum_notional_micro {
            return Err(AppError::ReferralIneligible);
        }

        let referrer_amount = policy.referrer_amount_micro;
        let referee_amount = policy.referee_amount_micro;
...
pub async fn grant_referral_on_paid_in_tx(
    ...
    let policy = required_referral_policy(tx).await?;
    let mut users = [referrer, referee];
    users.sort_unstable_by_key(|user| user.0);
    for user in users {
    ...
    if paid.market != paid_market || paid.notional_micro < policy.minimum_notional_micro {
        return Ok(false);
    }
...
mod tests {
    ...
    const REQUIRED_REFERRAL_CONFIGS: [&str; 5] = [
        "referral_min_notional_micro",
    ...
    fn remove_referral_config(store: &InMemoryStore, key: &str) {
        if key == "bonus_structure" {
            store.remove_money_text(key);
        } else {
            store.remove_money_i64(key);
        }
    }

    #[tokio::test]
    async fn standalone_grant_rejects_missing_required_policy_without_mutation() {
        let mut unexpected = Vec::new();
        for key in REQUIRED_REFERRAL_CONFIGS {
            let (store, referrer, referee, market, now) = qualifying_referral_fixture().await;
            remove_referral_config(&store, key);
            let before = store.snapshot();
            let result = GrantReferral { store: &store }
                .execute(GrantReferralCmd {
                    referee,
                    paid_market: market,
                    granted_at: now,
                })
                .await;
            let expected = Err(AppError::Store(StoreError::Invariant(
                "required money config missing or malformed",
            )));
            if result != expected {
                unexpected.push(format!("{key}: {result:?}"));
            }
            if store.snapshot() != before {
                unexpected.push(format!("{key}: committed state changed"));
            }
            let mut inspect = store.credit_convert_tx().await.unwrap();
            let lot_count = inspect.lots_for_user(referrer).await.unwrap().len()
                + inspect.lots_for_user(referee).await.unwrap().len();
            if lot_count != 0 {
                unexpected.push(format!("{key}: committed {lot_count} referral lots"));
            }
            inspect.commit().await.unwrap();
        }
        assert!(
            unexpected.is_empty(),
            "missing standalone referral policy did not fail closed: {unexpected:?}"
        );
    }

    #[tokio::test]
    async fn paid_hook_rejects_missing_required_policy_without_mutation() {
        let mut unexpected = Vec::new();
        for key in REQUIRED_REFERRAL_CONFIGS {
            let (store, referrer, referee, market, now) = qualifying_referral_fixture().await;
            remove_referral_config(&store, key);
            ...
            let result = grant_referral_on_paid_in_tx(tx.as_mut(), referee, market, now).await;
            let expected = Err(StoreError::Invariant(
                "required money config missing or malformed",
            ));
            if result != expected {
                unexpected.push(format!("{key}: {result:?}"));
            }
            let pending_lots = tx.lots_for_user(referrer).await.unwrap().len()
                + tx.lots_for_user(referee).await.unwrap().len();
            if pending_lots != 0 {
                unexpected.push(format!("{key}: buffered {pending_lots} referral lots"));
            }
            drop(tx);
            if store.snapshot() != before {
                unexpected.push(format!("{key}: committed state changed"));
        ...
        assert!(
            unexpected.is_empty(),
            "missing paid-hook referral policy did not fail closed: {unexpected:?}"
        );
    }

    #[tokio::test]
    async fn a_missing_referral_feature_flag_remains_disabled_not_malformed() {
        let (store, referrer, referee, market, now) = qualifying_referral_fixture().await;
        store.remove_money_flag("feature_referrals");
        let before = store.snapshot();
        assert_eq!(
        ...
        assert!(tx.lots_for_user(referrer).await.unwrap().is_empty());
        assert!(tx.lots_for_user(referee).await.unwrap().is_empty());
        drop(tx);
