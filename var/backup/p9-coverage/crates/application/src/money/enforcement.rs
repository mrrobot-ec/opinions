...
impl MoneyGateReads for dyn CreditIo + '_ {
    async fn gate_snapshot(
        ...
        let shadow_trade_cap_micro =
            self.config_i64("shadow_trade_cap_micro")
                .await?
                .ok_or(StoreError::Invariant(
                    "required money config missing or malformed",
                ))?;
        let shadow_deposit_cap_micro =
            self.config_i64("shadow_deposit_cap_micro")
                .await?
                .ok_or(StoreError::Invariant(
                    "required money config missing or malformed",
                ))?;
...
mod tests {
    ...
    #[tokio::test]
    async fn gate_snapshot_rejects_missing_required_shadow_caps() {
        let mut unexpected = Vec::new();
        for key in ["shadow_trade_cap_micro", "shadow_deposit_cap_micro"] {
            let store = InMemoryStore::new();
            ...
            let mut tx = store.deposit_admission_tx().await.unwrap();
            tx.lock_user(user).await.unwrap();
            let result = read_gate_snapshot(tx.as_mut(), user, OffsetDateTime::UNIX_EPOCH).await;
            if result
                != Err(StoreError::Invariant(
                    "required money config missing or malformed",
                ))
        ...
        assert!(
            unexpected.is_empty(),
            "missing shadow caps did not fail closed: {unexpected:?}"
        );
    }

    #[tokio::test]
    async fn zero_shadow_caps_remain_valid_and_deny_positive_amounts() {
        let store = InMemoryStore::new();
        store.set_money_i64("shadow_trade_cap_micro", 0);
        store.set_money_i64("shadow_deposit_cap_micro", 0);
        let user = store.add_user("zero-shadow-caps", OffsetDateTime::UNIX_EPOCH, 0);
        store.set_money_user(user, "shadow_limited", 1);
        let mut tx = store.deposit_admission_tx().await.unwrap();
        tx.lock_user(user).await.unwrap();
        let snapshot = read_gate_snapshot(tx.as_mut(), user, OffsetDateTime::UNIX_EPOCH)
            .await
            .unwrap();
        assert_eq!(snapshot.shadow_trade_cap_micro, 0);
        assert_eq!(snapshot.shadow_deposit_cap_micro, 0);
        for mutation in [MoneyMutation::PlaceTrade, MoneyMutation::DepositAdmit] {
            assert_eq!(
                decide_money_mutation(&snapshot, mutation, 1),
