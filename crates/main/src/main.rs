...
// D35's live contract gates committed-event-to-WebSocket delivery at p95
// <= 100ms. Polling at the entire budget leaves no time for the query,
// transaction commit, broadcast, or socket delivery.
fn outbox_relay_tick() -> std::time::Duration {
...
async fn main() -> anyhow::Result<()> {
    ...
    tokio::spawn(relay.run(outbox_relay_tick()));
...
mod tests {
    ...
    #[test]
    fn websocket_relay_reserves_half_the_delivery_budget_for_processing() {
        assert!(
            outbox_relay_tick() <= std::time::Duration::from_millis(50),
            "a 100ms poll consumes the entire 100ms WebSocket p95 budget before processing"
        );
    }
