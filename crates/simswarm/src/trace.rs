...
impl SeriesContract {
    ...
    pub fn from_manifest(manifest: &crate::manifest::RunManifest) -> Self {
        ...
        let spike =
            usize::try_from(manifest.scenario.schedule.close_spike_ticks).unwrap_or(usize::MAX);
        let expected_ws_delivery = spike.div_ceil(60);
...
mod tests {
    ...
    fn series_contract_is_derived_from_the_manifest_roster() {
        ...
        assert_eq!(contract.expected_ws_delivery, 2);
    ...
    #[test]
    fn pinned_spike_requires_two_real_websocket_samples() {
        let manifest = crate::manifest::RunManifest::profile(crate::manifest::Profile::Smoke, 3);
        assert_eq!(manifest.scenario.schedule.close_spike_ticks, 120);
        let contract = SeriesContract::from_manifest(&manifest);
        assert_eq!(contract.expected_ws_delivery, 2);

        let mut log = LatencyLog::default();
        for _ in 0..contract.expected_trade_2xx {
            log.record("trade-confirm", 200, 10);
        }
        log.record("ws-delivery", 200, 10);
        let report = log.slo_report_for(true, &contract);
        assert!(matches!(
            evaluate_slo(&report),
            Err(SloGateError::ThinSeries {
                series: "ws-delivery",
