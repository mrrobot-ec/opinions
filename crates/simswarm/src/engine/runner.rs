...
mod tests {
    ...
    #[test]
    fn malformed_or_backdated_trade_frames_are_not_latency_evidence() {
        let base = OffsetDateTime::UNIX_EPOCH;
        let mut tracker = WsDeliveryTracker::default();
        let mut latency = LatencyLog::default();

        for frame in [
            json!({"type":"trade", "created_at":"1970-01-01T00:00:00Z"}),
            json!({"type":"trade", "outbox_seq":7}),
            json!({"type":"trade", "outbox_seq":7, "created_at":"not-a-timestamp"}),
            json!({
                "type":"trade",
                "outbox_seq":7,
            ...
            tracker.observe(&frame, base, &mut latency);
        }

        assert!(latency.successful("ws-delivery").is_empty());
    }
