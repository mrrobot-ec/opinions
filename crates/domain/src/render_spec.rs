...
mod tests {
    ...
    proptest! {
        ...
        #[test]
        fn spec_builders_apply_pinned_wrapping_and_layout(question in ".{0,200}") {
            let poster = poster_spec(&question);
            let video = video_spec(&question);
            let poster_lines: Vec<_> = wrap_chars(&question, 24)
                .into_iter()
                .take(8)
                .map(|line| escape_text(&line))
                .collect();
            let video_lines: Vec<_> = wrap_chars(&question, 22)
                .into_iter()
                .take(10)
                .map(|line| escape_text(&line))
                .collect();

            prop_assert_eq!((poster.width, poster.height), (1080, 1350));
            prop_assert_eq!((video.width, video.height), (1080, 1920));
            prop_assert_eq!(
                poster.runs[1..poster.runs.len() - 1]
                    .iter()
