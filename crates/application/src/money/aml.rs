...
pub struct AmlEvaluation {
    ...
    pub deposit_velocity_saturated: bool,
    pub withdraw_velocity: i64,
    pub withdraw_velocity_saturated: bool,
...
pub fn evaluate_aml(existing: &[AmlLeg], candidate: &AmlLeg, policy: &AmlPolicy) -> AmlEvaluation {
    ...
    let structuring_dest = legs
        ...
        .count();
    let (deposit_velocity, deposit_velocity_saturated) = legs
        .iter()
        .filter(|leg| leg.user == candidate.user && matches!(leg.direction, AmlDirection::Deposit))
        .map(|leg| leg.amount_micro)
        .fold((0_i64, false), |(total, saturated), amount| {
            total.checked_add(amount).map_or_else(
                || (total.saturating_add(amount), true),
                |next| (next, saturated),
            )
        });
    let (withdraw_velocity, withdraw_velocity_saturated) = legs
        .iter()
        .filter(|leg| {
    ...
    AmlEvaluation {
        ...
        deposit_velocity,
        deposit_velocity_saturated,
        withdraw_velocity,
        withdraw_velocity_saturated,
        flags,
    }
}
    ...
    for kind in &evaluation.flags {
        ...
        tx.insert_aml_flag(AmlFlag {
            ...
            evidence: json!({
                ...
                "deposit_velocity_saturated": evaluation.deposit_velocity_saturated,
                "withdraw_velocity": evaluation.withdraw_velocity,
                "withdraw_velocity_saturated": evaluation.withdraw_velocity_saturated,
                "leg_id": candidate.id,
            }),
            open: true,
            at: candidate.at,
...
mod tests {
    ...
    fn velocity_totals_saturate_and_flag_when_history_exceeds_i64() {
        ...
        assert!(deposit_eval.deposit_velocity_saturated);
        assert!(!deposit_eval.withdraw_velocity_saturated);
        ...
        assert!(withdrawal_eval.withdraw_velocity_saturated);
        assert!(!withdrawal_eval.deposit_velocity_saturated);
        assert!(withdrawal_eval.flags.contains(&AmlKind::WithdrawVelocity));
    }

    #[tokio::test]
    async fn persisted_evidence_marks_saturated_velocity_totals_as_lower_bounds() {
        for (label, prior, candidate, rule, saturated_key, exact_key) in [
            (
                "deposit",
                deposit(UserId(Uuid::from_u128(31)), "source", i64::MAX, 1),
                deposit(UserId(Uuid::from_u128(31)), "source", 1, 2),
                AmlKind::DepositVelocity,
                "deposit_velocity_saturated",
                "withdraw_velocity_saturated",
            ),
            (
                "withdrawal",
                withdraw(UserId(Uuid::from_u128(32)), "dest", i64::MAX, 3),
                withdraw(UserId(Uuid::from_u128(32)), "dest", 1, 4),
                AmlKind::WithdrawVelocity,
                "withdraw_velocity_saturated",
                "deposit_velocity_saturated",
            ),
            ...
            let user = store.add_user(label);
            let prior = AmlLeg { user, ..prior };
            let candidate = AmlLeg { user, ..candidate };
            let mut seed = store.compliance_tx().await.unwrap();
            seed.lock_user(user).await.unwrap();
            seed.record_aml_leg(prior).await.unwrap();
            seed.commit().await.unwrap();

            evaluate_at_request(&store, candidate, AmlPolicy::seed())
                .await
                .unwrap();
            let mut inspect = store.compliance_tx().await.unwrap();
            let flags = inspect.open_aml_flags(user).await.unwrap();
            let evidence = &flags
                .iter()
                .find(|flag| flag.rule == rule)
                .unwrap()
                .evidence;
            assert_eq!(
                evidence.get(saturated_key),
                Some(&Value::Bool(true)),
                "{label} evidence must disclose that i64::MAX is a lower bound"
            );
            assert_eq!(
                evidence.get(exact_key),
                Some(&Value::Bool(false)),
                "the unsaturated {label} counterpart remains exact"
            );
