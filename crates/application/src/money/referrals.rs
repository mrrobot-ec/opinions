...
mod tests {
    ...
    async fn standalone_grant_rejects_missing_required_policy_without_mutation() {
        for key in REQUIRED_REFERRAL_CONFIGS {
            ...
            assert_eq!(result, expected, "missing {key} must fail closed");
            assert_eq!(store.snapshot(), before, "{key} changed committed state");
            ...
            assert_eq!(lot_count, 0, "{key} committed referral lots");
    ...
    async fn paid_hook_rejects_missing_required_policy_without_mutation() {
        for key in REQUIRED_REFERRAL_CONFIGS {
            ...
            assert_eq!(pending_lots, 0, "{key} buffered referral lots");
            drop(tx);
            assert_eq!(store.snapshot(), before, "{key} changed committed state");
        }
    }
    ...
    async fn bind_waits_for_both_participant_locks_in_uuid_order() {
        ...
        let mut users = [first, second];
        users.sort_unstable_by_key(|user| user.0);
        let [referrer, referee] = users;
        store.set_phone_verified(referee, true);
