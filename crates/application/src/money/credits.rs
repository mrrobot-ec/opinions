...
impl<S: Store> GrantCredit<'_, S> {
    ...
    pub async fn execute_as(
        ...
        let mint_cap =
            tx.config_i64("bonus_mint_daily_cap_micro")
                .await?
                .ok_or(StoreError::Invariant(
...
mod tests {
    ...
    #[tokio::test]
    async fn grant_rejects_a_missing_required_bonus_mint_cap_without_mutation() {
        let store = crate::fakes::InMemoryStore::new();
        ...
        store.remove_money_i64("bonus_mint_daily_cap_micro");
        let before = store.snapshot();
        let key = "missing-required-mint-cap";
        ...
        assert!(inspect.lots_for_user(user).await.unwrap().is_empty());
        assert_eq!(inspect.txn_by_key(key).await.unwrap(), None);
