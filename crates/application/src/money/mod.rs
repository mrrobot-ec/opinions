...
pub trait ComplianceTx: Committable + Send {
    ...
    /// Serialize one inbox key for the rest of this transaction — the
    /// `ComplianceTx` instance of the codex-B3 rule already stated on
    /// [`crate::ports::IdempotencyGuard::serialize_key`]: it MUST be the first
    /// call in the write tx, so the read-or-create that follows is race-free.
    ///
    /// The inbox key, not the user, is the thing that has to serialize. Two
    /// deliveries sharing `(provider, event_id)` may carry different
    /// `provider_ref`s and therefore resolve to DIFFERENT users, so a class-2
    /// user lock does not order them: both read absent and the loser hits the
    /// unique violation instead of the D33 `Replay`/typed-conflict answer.
    /// Postgres implements this as a class-1 `pg_advisory_xact_lock`; the fake
    /// holds an owned per-key guard for the life of the transaction.
    ///
    /// # Errors
    ...
    /// Inbox algebra reads, in the SAME unit of work as the effect. Accepting
    /// a delivery in one transaction and applying its KYC effect in another
    /// strands the inbox row when the effect fails: every later retry is a
    /// Replay and the effect is skipped forever (codex-p7r1:73-75 required the
    /// coupled commit). These mirror the `ComplianceAdminTx` methods of the
    /// same name; both traits are implemented by one concrete transaction.
    async fn inbox_get(
        &mut self,
        provider: &str,
        event_id: &str,
    ) -> Result<Option<admin::InboxRecord>, StoreError>;
    async fn inbox_insert(
        &mut self,
        record: admin::InboxRecord,
    ) -> Result<admin::InboxRecord, StoreError>;
}
...
/// Narrow role carried by every transaction that can be the user's next
/// money lock. A ready bonus lot converts and any receivable is collected in
/// that same transaction before the requested mutation continues.
#[async_trait]
pub trait LazyCreditConversion: Send {
    /// # Errors
    /// Ledger, lot, receivable, or backend failures abort the caller's whole
    /// transaction.
