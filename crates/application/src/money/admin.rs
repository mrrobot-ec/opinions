...
pub trait ComplianceAdminTx: ComplianceTx {
    ...
    // `inbox_get` / `inbox_insert` moved down to the `ComplianceTx` supertrait
    // so a delivery's acceptance and its effect can share one commit. They
    // remain callable here unchanged.
...
pub async fn accept_inbox(
    ...
    // codex B3: the inbox key serializes first, so the read-or-create below is
    // race-free. The legacy two-transaction route keeps the same guarantee as
    // `accept_and_apply_inboxed_kyc`; a concurrent duplicate now answers
    // Replay instead of surfacing a raw unique violation.
    tx.serialize_inbox(&incoming.provider, &incoming.event_id)
        .await?;
