...
pub(super) struct Phase7Committed {
    ...
    pub aml_legs: Vec<crate::money::AmlLeg>,
    pub aml_flags: Vec<crate::money::AmlFlag>,
...
fn apply_phase7(pending: &Phase7Pending, next: &mut State) -> Result<(), StoreError> {
    ...
    for row in &pending.deposits {
        ...
        next.phase7
            .deposit_sigs
            .insert(row.chain_sig.clone(), row.id.0);
    ...
    next.phase7
        .decisions
        .extend(pending.decisions.iter().cloned());
    for leg in &pending.aml_legs {
        if next
            .phase7
            .aml_legs
            .iter()
            .any(|existing| existing.id == leg.id)
        {
            return Err(StoreError::Conflict("deposit aml candidate"));
        }
        next.phase7.aml_legs.push(leg.clone());
    }
    next.phase7
        .aml_flags
        .extend(pending.aml_flags.iter().cloned());
    next.phase7
        .proposals
        .extend(pending.proposals.iter().cloned());
    Ok(())
}
...
impl Default for InMemoryStore {
    fn default() -> Self {
        ...
        state.phase7.flags.insert("pause_deposits".into(), false);
        state.phase7.ints.insert("deposit_kyc_tier".into(), 1);
        state
            .phase7
            .ints
            .insert("bonus_mint_daily_cap_micro".into(), 500_000_000);
        state
            .phase7
            .ints
            .insert("referral_min_notional_micro".into(), 10_000_000);
        state
            .phase7
            .ints
            .insert("credit_referral_referrer_micro".into(), 5_000_000);
        state
            .phase7
            .ints
            .insert("credit_referral_referee_micro".into(), 5_000_000);
        state.phase7.ints.insert(
            "shadow_trade_cap_micro".into(),
            crate::money::PUBLISHED_TIER0_CAP_MICRO,
        );
        state.phase7.ints.insert(
            "shadow_deposit_cap_micro".into(),
            crate::money::PUBLISHED_TIER0_CAP_MICRO,
        );
        state
            .phase7
            .strings
            .insert("bonus_structure".into(), "real_money".into());
        Self {
...
mod phase7_commit_semantics_tests {
    use super::*;
    use crate::money::{
        AllocationFact, AllocationKind, CreditLotRow, DepositMachineRow, GrantClass,
    };
    ...
    fn concurrent_credit_commits_preserve_lot_allocation_and_terminal_identities() {
        ...
        assert_eq!(
            apply_phase7(
                &Phase7Pending {
                    allocations: vec![allocation(22, "same-key", AllocationKind::Allocated, None,)],
                    ..Phase7Pending::default()
                },
                &mut state,
            ),
        ...
        let mut state = State::default();
        state.phase7.allocations.push(allocation(
            23,
            "first-split",
    ...
    fn concurrent_deposit_and_referral_commits_preserve_unique_identities() {
        ...
        state
            .phase7
            .deposit_sigs
            .insert("same-signature".into(), id(28));
        assert_eq!(
            apply_phase7(
                &Phase7Pending {
        ...
        assert_eq!(
            apply_phase7(
                &Phase7Pending {
                    binds: vec![(id(35), UserId(id(36)), UserId(id(34)), "second-bind".into(),)],
                    ..Phase7Pending::default()
                },
                &mut state,
            ),
