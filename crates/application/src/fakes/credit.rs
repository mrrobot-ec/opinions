...
mod credit_fake_semantics_tests {
    ...
    #[tokio::test]
    async fn deposit_aml_reuses_an_existing_open_rule_instead_of_duplicating_it() {
        ...
        let now = OffsetDateTime::UNIX_EPOCH + time::Duration::days(1);
        {
            ...
            for index in 0..3_u8 {
                state.phase7.aml_legs.push(AmlLeg {
                    id: id(10 + u128::from(index)),
                    user,
                    dest: "shared-source".into(),
                    amount_micro: 100_000_000,
                    at: now - time::Duration::seconds(i64::from(index)),
                    direction: AmlDirection::Deposit,
                });
            }
            state.phase7.aml_flags.push(AmlFlag {
                id: id(20),
                user,
                rule: crate::money::AmlKind::Structuring,
        ...
        assert!(tx
            ...
            .await
            .unwrap_or_else(|error| panic!("deposit AML evaluation failed: {error}")));
        tx.commit()
            .await
            .unwrap_or_else(|error| panic!("deposit transaction commit failed: {error}"));
        let state = store.shared.state.lock();
        assert_eq!(state.phase7.aml_flags.len(), 1);
        assert_eq!(state.phase7.aml_legs.len(), 4);
    }

    #[tokio::test]
    async fn concurrent_duplicate_deposit_aml_candidates_conflict_at_commit() {
        let store = InMemoryStore::new();
        let user = UserId(id(1));
        let deposit = DepositId(id(40));
        let mut first = store
            .deposit_admission_tx()
            .await
            .unwrap_or_else(|error| panic!("first deposit transaction failed: {error}"));
        let mut second = store
            .deposit_admission_tx()
            .await
            .unwrap_or_else(|error| panic!("second deposit transaction failed: {error}"));
        ...
        first
            .commit()
            .await
            .unwrap_or_else(|error| panic!("first deposit transaction commit failed: {error}"));
        assert_eq!(
            second.commit().await,
