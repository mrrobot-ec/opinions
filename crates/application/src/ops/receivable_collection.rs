...
use crate::ports::{Clock, LedgerWriter, ReceivableCollectionIo, Store, WriteOffProposal};
...
where
    T: LedgerWriter + ReceivableCollectionIo + ?Sized,
{
...
mod tests {
    ...
    #[tokio::test]
    async fn deposit_collection_rejects_an_overflowed_receivable_total() {
        let store = InMemoryStore::new();
        let (user, _) = open_receivable(&store, i64::MAX).await;
        let market = store
            .add_market(
                "recv-overflow-market",
                domain::market::MarketState::Voided,
                OffsetDateTime::UNIX_EPOCH,
                OffsetDateTime::UNIX_EPOCH,
                domain::money::MicroShares(1),
        ...
        let mut setup = store.unwind_tx().await.unwrap();
        setup.insert_receivable(second).await.unwrap();
        setup
            .insert_receivable_movement(ReceivableMovement {
                id: Uuid::new_v4(),
                receivable: second.id,
                ...
                actor: "test".into(),
                cash_txn: None,
                idempotency_key: "overflow-second-open".into(),
            })
            .await
            .unwrap();
        ...
        let mut collect = store.deposit_tx().await.unwrap();
        assert_eq!(
            auto_collect(collect.as_mut(), user, "overflow", "machine:test")
                .await
                .unwrap_err(),
            AppError::Overflow
        );
