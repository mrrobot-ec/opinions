...
pub async fn admit_observed<S: Store>(
    ...
    if tx
        .evaluate_deposit_aml_candidate(row.id, user, &row.source_address, row.amount.0, now)
        .await?
    {
        return hold_admission(tx, &row, "aml_open", 0, actor, now).await;
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn deposit_admission_holds_a_shared_source_structuring_flag() {
        let store = InMemoryStore::new();
        let now = OffsetDateTime::UNIX_EPOCH + time::Duration::days(50_000);
        let mut users = Vec::new();

        for index in 0..4_u8 {
            let user =
                InMemoryStore::add_user(&store, &format!("deposit-aml-user-{index}"), now, 0);
            users.push(user);
            let observed = ObservedDeposit {
                user: Some(user),
                amount: MicroUsd(100_000_000 + i64::from(index)),
                chain_sig: format!("deposit-aml-sig-{index}"),
                source_address: "shared-source-counterparty".into(),
                dest_address: format!("distinct-treasury-destination-{index}"),
                mint: "usdc-mint".into(),
                slot: i64::from(index),
            };
            observe_finalized(&store, &observed).await.unwrap();
            admit_observed(
                &store,
                &observed.chain_sig,
                now + time::Duration::seconds(i64::from(index)),
                &AdminContext::Machine,
                false,
            )
            .await
        ...
        for (index, user) in users.iter().take(3).enumerate() {
            assert_eq!(
                store.balance_of(OwnerRef::User(*user), Currency::Usdc),
                Some(MicroUsd(100_000_000 + i64::try_from(index).unwrap()))
            );
        }
        assert_eq!(
            store.balance_of(OwnerRef::User(users[3]), Currency::Usdc),
            None,
            "the fourth in-band deposit from one source must remain in suspense"
        );
        assert_eq!(
            store.balance_of(OwnerRef::DepositSuspense, Currency::Usdc),
            Some(MicroUsd(100_000_003))
        );
        assert_eq!(store.deposit_aml_legs().len(), 4);
        ...
        .unwrap();
        assert!(replay.replayed);
        let reevaluated = reevaluate_held(
            &store,
            "deposit-aml-sig-3",
        ...
        .await
        .unwrap();
        assert!(!reevaluated.replayed);
        let legs = store.deposit_aml_legs();
        assert_eq!(legs.len(), 4, "replay and reevaluation must not add a leg");
        assert_eq!(legs[3].dest, "shared-source-counterparty");
    }
