...
pub async fn run<S: Store + ?Sized>(store: &S) -> Result<InvariantReport, AppError> {
    ...
    for currency in [Currency::Usdc, Currency::UsdcCredit] {
        ...
        if external.checked_neg() != Some(internal) {
            mirror_failures.push(format!(
                "{currency:?}: external {external} + internal {internal} is not zero"
            ));
        }
    }
    let pass = mirror_failures.is_empty();
    identities.push(result(
        "external_mirrors_internal",
        pass,
        failure_detail(pass, mirror_failures.join("; ")),
    ));
...
impl MoneyIdentityFacts {
    ...
    pub fn from_balances(balances: &[crate::model::AccountBalanceRow]) -> Self {
        let sum = |owner: OwnerType| {
            balances
                .iter()
                .filter(|row| row.owner_type == owner && row.currency == Currency::Usdc)
...
mod tests {
    ...
    #[test]
    fn money_identity_facts_ignore_non_cash_currency_balances() {
        let facts = MoneyIdentityFacts::from_balances(&[
            crate::model::AccountBalanceRow {
                owner_type: OwnerType::Withheld,
                currency: Currency::Usdc,
                balance_micro: 3,
            ...
            crate::model::AccountBalanceRow {
                owner_type: OwnerType::Withheld,
                currency: Currency::UsdcCredit,
                balance_micro: 7,
            },
        ]);
        assert_eq!(facts.withheld_balance_micro, 3);
    }

    #[test]
    fn account_balance_snapshot_rows_keep_wide_aggregates() {
        let row = crate::model::AccountBalanceRow {
            owner_type: OwnerType::User,
            currency: Currency::Usdc,
            balance_micro: 0,
        };
        assert_eq!(
            std::mem::size_of_val(&row.balance_micro),
    ...
    fn aggregate_money_identities_handle_totals_larger_than_i64() {
        let mut facts = MoneyIdentityFacts::from_balances(&[
            crate::model::AccountBalanceRow {
                owner_type: OwnerType::Withheld,
                currency: Currency::Usdc,
                balance_micro: i128::from(i64::MAX),
            },
            crate::model::AccountBalanceRow {
                owner_type: OwnerType::Withheld,
                currency: Currency::Usdc,
        ...
        assert_eq!(
            facts.withheld_balance_micro.to_string(),
            "9223372036854775808"
        );
        assert_eq!(
            facts.suspense_balance_micro.to_string(),
            "9223372036854775808"
        );
        assert_eq!(
