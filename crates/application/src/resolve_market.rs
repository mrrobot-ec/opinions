...
mod tests {
    ...
    #[tokio::test]
    async fn resolution_fails_closed_when_participants_never_stabilize() {
        let fx = Fixture::seeded(1).await;
        let mut users = (0..9)
            .map(|index| fx.store.add_user(&format!("drift-{index}"), t0(), 1))
            .collect::<Vec<_>>();
        users.sort_unstable_by_key(|user| user.0);
        let referrers = [users[3], users[2], users[1], users[0]];
        let voters = [users[4], users[5], users[6], users[7], users[8]];
        for (index, voter) in voters.into_iter().enumerate() {
            ...
            .execute(CastVoteCmd {
                ...
                side: Side::Yes,
                crowd_guess_pct: 80,
                idempotency_key: format!("drift-vote-{index}"),
                cast_ip: None,
                device_hash: None,
            })
            .await
            .unwrap();
            fx.buy(voter, Side::Yes, 1_000_000).await;
        }
        for voter in voters.into_iter().skip(1) {
            fx.store.set_phone_verified(voter, true);
        }
        fx.close().await;

        let mut blocker = fx.store.credit_convert_tx().await.unwrap();
        blocker.lock_user(voters[0]).await.unwrap();
        let resolver = fx.resolve();
        let resolution = resolver.execute(ResolveMarketCmd {
            market: fx.market,
            curator_override: None,
        ...
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(25), &mut resolution)
                .await
                .is_err(),
            "resolution must wait at the first participant lock"
        );

        for (round, (referrer, referee)) in
            referrers.into_iter().zip(voters[1..].iter()).enumerate()
        {
            let bind_action = crate::money::referrals::BindReferral { store: &fx.store };
            let bind = bind_action.execute(crate::money::referrals::BindReferralCmd {
                referrer,
                referee: *referee,
                bind_key: format!("drift-bind-{round}"),
                idempotency_key: format!("drift-bind-command-{round}"),
            });
            ...
            let mut next_blocker = fx.store.credit_convert_tx().await.unwrap();
            next_blocker.lock_user(referrer).await.unwrap();
            drop(blocker);
            if round + 1 < RESOLUTION_PARTICIPANT_LOCK_ATTEMPTS {
                assert!(
                    tokio::time::timeout(std::time::Duration::from_millis(25), &mut resolution,)
                    .await
                    .is_err(),
                    "round {round}: resolution must retry and wait on the newly introduced participant"
                );
            }
        ...
        assert_eq!(
            ...
            AppError::Store(StoreError::Invariant(
                "resolution participant set did not stabilize"
            ))
        );
        drop(blocker);
    }
