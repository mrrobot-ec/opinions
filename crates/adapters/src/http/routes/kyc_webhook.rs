...
mod tests {
    ...
    async fn an_unsigned_webhook_is_refused_before_any_store_access() {
        ...
        let compliance_error = application::money::ComplianceStore::compliance_tx(&NoStoreAccess)
            .await
            .err()
            .unwrap();
        assert_eq!(
            compliance_error,
            application::error::StoreError::Unavailable(
                "store touched before signature verification"
            )
        );
        let admin_error = ComplianceAdminStore::admin_tx(&NoStoreAccess)
            .await
            .err()
            .unwrap();
        assert_eq!(
