...
#[utoipa::path(
    get,
    path = "/markets/{id_or_slug}/my-vote",
    tag = "votes",
    params(
        ("id_or_slug" = String, Path, description = "Market UUID or slug"),
        ("x-user-id" = Uuid, Header, description = "Authenticated local-demo viewer id"),
    ),
    responses(
        (status = 200, body = ViewerVoteStatusDto),
        (status = 401, body = ApiError),
        (status = 403, body = ApiError),
        (status = 404, body = ApiError),
    )
)]
async fn viewer_vote_status<S: Store + MarketQueries + 'static>(
    State(state): State<AppState<S>>,
    Path(id_or_slug): Path<String>,
    headers: HeaderMap,
    axum::extract::RawQuery(raw_query): axum::extract::RawQuery,
) -> ApiResult<Json<ViewerVoteStatusDto>> {
    ...
    if raw_query.is_some() {
        return Err(ErrorResponse::new(
            ...
            "CrossUserVoteQuery",
            "my-vote does not accept a target user",
        ));
    }
    let market = state
        .inner
    ...
    let has_voted = state
        .inner
        .store
        .user_voted(viewer, market.id)
        .await
        .map_err(application::error::AppError::from)?;
    Ok(Json(ViewerVoteStatusDto { has_voted }))
}
