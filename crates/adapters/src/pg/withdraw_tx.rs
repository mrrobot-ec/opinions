//! Postgres withdrawal unit of work (D31). Its narrow wrapper owns the shared
//! `PgTx`, so W1 can reuse D32 conversion/collection inside the same transaction.
...
use application::money::{
    evaluate_aml, AmlDirection, AmlLeg, AmlPolicy, ConvertCollectReceipt, CreditIo,
    LazyCreditConversion,
};
...
#[async_trait]
impl LazyCreditConversion for PgWithdrawTx {
    async fn convert_then_collect(
        ...
        CreditIo::convert_then_collect(&mut self.inner, user, now, key).await
    }
}
...
impl WithdrawIo for PgWithdrawTx {
    ...
    async fn user_cash(&mut self, user: UserId) -> Result<i64, StoreError> {
        let id = user_account(&mut self.inner.tx, user).await?;
        let bal: i64 = sqlx::query_scalar(
            "select coalesce(sum(amount_micro),0)::bigint from ledger_entries where account_id = $1",
        )
        .bind(id)
    ...
    async fn user_money_view(&mut self, user: UserId) -> Result<UserMoneyView, StoreError> {
        let row = sqlx::query("select status, kyc_tier from users where id = $1")
            .bind(user.0)
            .fetch_optional(&mut *self.inner.tx)
            .await
            .map_err(db_error)?
    ...
    async fn apply_hold(
        ...
        let user_account = user_account(&mut self.inner.tx, user).await?;
        let withheld = withheld_account(&mut self.inner.tx).await?;
        two_leg(
            &mut self.inner.tx,
            key,
            "withdrawal",
    ...
    async fn apply_settle(&mut self, amount_micro: i64, key: &str) -> Result<Uuid, StoreError> {
        let withheld = withheld_account(&mut self.inner.tx).await?;
        let external = external_account(&mut self.inner.tx).await?;
        two_leg(
            &mut self.inner.tx,
            key,
            "withdrawal",
...
impl OutboundIo for PgWithdrawTx {
    async fn insert_outbound_payment(
        ...
        insert_payment(&mut self.inner.tx, payment).await
    }
    ...
    async fn outbound_by_subject(
        ...
        payment_by_subject(&mut self.inner.tx, subject, subject_id).await
    }

    async fn insert_attempt(&mut self, attempt: &OutboundAttemptRow) -> Result<(), StoreError> {
        insert_attempt_row(&mut self.inner.tx, attempt).await
    }
    ...
    async fn save_attempt(&mut self, attempt: &OutboundAttemptRow) -> Result<(), StoreError> {
        save_attempt_row(&mut self.inner.tx, attempt).await
    }
