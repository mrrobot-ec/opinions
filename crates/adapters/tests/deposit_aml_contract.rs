...
async fn dust_below_the_structuring_floor_never_accumulates_into_a_flag() -> TestResult {
    ...
    // A converted credit is a withdrawal-direction AML leg. Keep it below
    // the band while proving the deposit-side query decodes both directions.
    sqlx::query(
        "insert into credit_grant_lots
            (user_id, source, amount_micro, grant_class, policy_version, converted_at)
         values ($1, 'coverage', 5000000, 'real_money', 'coverage-v1', now())",
    )
    .bind(scratch.user.0)
    .execute(&scratch.pool)
    .await?;
...
#[tokio::test]
async fn malformed_or_invalid_required_aml_policy_fails_closed() -> TestResult {
    let scratch = scratch("policy").await?;
    let deposit = observed(&scratch, IN_BAND_MICRO).await?;
    ...
    for key in [
        ...
        let original: serde_json::Value =
            sqlx::query_scalar("select value from config_entries where key = $1")
                ...
                .await?;
        sqlx::query("update config_entries set value = '\"malformed\"'::jsonb where key = $1")
            .bind(key)
            .execute(&scratch.pool)
            .await?;
        let mut tx = scratch.store.deposit_admission_tx().await?;
        ...
        assert_eq!(
            ...
            Err(StoreError::Backend(format!(
                "withdraw config {key} is not an integer"
            ))),
            "{key} must be mandatory and integer-valued"
        );
        drop(tx);
    ...
    sqlx::query(
        "update config_entries
            set value = to_jsonb(500000000::bigint)
          where key = 'aml_structuring_floor_micro'",
    )
    .execute(&scratch.pool)
    .await?;
    let mut tx = scratch.store.deposit_admission_tx().await?;
    let invalid = tx
        ...
        .await;
    assert_eq!(invalid, Err(StoreError::Invariant("invalid AML policy")));
    drop(tx);
