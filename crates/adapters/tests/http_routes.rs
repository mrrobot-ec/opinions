...
fn my_vote_request(market_ref: &str, viewer: UserId) -> axum::http::Request<axum::body::Body> {
    axum::http::Request::builder()
        .uri(format!("/markets/{market_ref}/my-vote"))
        .header("x-demo-token", DEMO)
        .header("x-user-id", viewer.0.to_string())
...
#[tokio::test]
async fn my_vote_returns_true_for_the_authenticated_viewer_after_voting() {
    let (_app, store, voter, slug) = setup();
    let market = store.market_by_ref(&slug).await.unwrap();
    store.record_vote(voter, market.id, Side::Yes);

    let response = app_from(store)
        .oneshot(my_vote_request(&slug, voter))
        .await
        .unwrap();

    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(body_json(response).await, json!({"has_voted": true}));
}

#[tokio::test]
async fn my_vote_returns_false_for_an_authenticated_viewer_without_a_vote() {
    let (_app, store, viewer, slug) = setup();

    let response = app_from(store)
        .oneshot(my_vote_request(&slug, viewer))
        .await
        .unwrap();

    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(body_json(response).await, json!({"has_voted": false}));
}

#[tokio::test]
async fn my_vote_requires_a_scoped_viewer_header() {
    let (_app, store, _viewer, slug) = setup();
    let request = axum::http::Request::builder()
        .uri(format!("/markets/{slug}/my-vote"))
        .header("x-demo-token", DEMO)
        .body(axum::body::Body::empty())
        .unwrap();

    let response = app_from(store).oneshot(request).await.unwrap();

    assert_eq!(response.status(), StatusCode::UNAUTHORIZED);
    assert_eq!(body_json(response).await["code"], "ViewerRequired");
}

#[tokio::test]
async fn my_vote_refuses_a_cross_user_selector() {
    let (_app, store, voter, slug) = setup();
    let market = store.market_by_ref(&slug).await.unwrap();
    store.record_vote(voter, market.id, Side::No);
    let viewer = store.add_user("vote-status-viewer", t0(), 0);
    let request = axum::http::Request::builder()
        .uri(format!("/markets/{slug}/my-vote?user_id={}", voter.0))
        .header("x-demo-token", DEMO)
        .header("x-user-id", viewer.0.to_string())
        .body(axum::body::Body::empty())
        .unwrap();
    ...
    assert_eq!(body_json(response).await["code"], "CrossUserVoteQuery");
}

#[tokio::test]
async fn my_vote_live_response_never_leaks_the_vote_side() {
    let (_app, store, voter, slug) = setup();
    let market = store.market_by_ref(&slug).await.unwrap();
    store.record_vote(voter, market.id, Side::No);
    ...
    assert_eq!(response, json!({"has_voted": true}));
    assert!(response.get("side").is_none());
    assert!(response.get("crowd_guess_pct").is_none());
    assert!(response.get("seq").is_none());
}
